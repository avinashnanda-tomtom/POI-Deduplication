...
def combine(log):
    ...
    final_candidates =candidates_cosine
    ...
    final_candidates = pd.merge(final_candidates, df[["placeId","longitude","latitude"]],  how='left', left_on=[
                'placeId1'], right_on=['placeId'])
    final_candidates.drop('placeId',inplace=True,axis=1)
    final_candidates = pd.merge(final_candidates, df[["placeId","longitude","latitude"]],  how='left', left_on=[
