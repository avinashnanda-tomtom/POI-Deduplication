...
def create_edit_features_em(df_pairs):
    """Create all distance based features.

    Args:
        df_pairs (dataframe): pairs of poi dataframe.

    Returns:
        dataframe: dataframe with distance calculated.
    """
    columns = [
        "sourceNames",
        "houseNumber",
    ...
    for c in columns:
        if c in ["sourceNames", "category", "streets", "cities"]:

            df_pairs[f"{c}_jaro"] = np.vectorize(jaro)(df_pairs[c + "1"],
                                                       df_pairs[c + "2"])
            df_pairs[f"{c}_set_ratio"] = (np.vectorize(token_set_ratio)(
                df_pairs[c + "1"], df_pairs[c + "2"]))
            df_pairs[f"{c}_WRatio"] = (np.vectorize(WRatio)(df_pairs[c + "1"],
                                                            df_pairs[c + "2"]))
            df_pairs[f"{c}_ratio"] = (np.vectorize(ratio)(df_pairs[c + "1"],
                                                          df_pairs[c + "2"]))
            df_pairs[f"{c}_qratio"] = (np.vectorize(QRatio)(df_pairs[c + "1"],
                                                            df_pairs[c + "2"]))
            df_pairs[f"{c}_nleven"] = np.vectorize(leven)(df_pairs[c + "1"],
                                                          df_pairs[c + "2"])
            df_pairs[f"{c}_Dice"] = np.vectorize(strike_a_match)(
                df_pairs[c + "1"], df_pairs[c + "2"])
            df_pairs[f"{c}_lcs"] = np.vectorize(lcs)(df_pairs[c + "1"],
                                                     df_pairs[c + "2"])
            df_pairs[f"{c}_davies"] = np.vectorize(davies)(df_pairs[c + "1"],
                                                           df_pairs[c + "2"])
            df_pairs[f"{c}_soundex"] = np.vectorize(get_phonetic_soundex)(
                df_pairs[c + "1"], df_pairs[c + "2"])
            df_pairs[f"{c}_metaphone"] = np.vectorize(get_phonetic_metaphone)(
                df_pairs[c + "1"], df_pairs[c + "2"])
        elif c == "supplier":
            df_pairs[f"{c}_match"] = (df_pairs[c + "1"]
                                      == df_pairs[c + "2"]) * 1
    ...
    df_pairs = df_pairs.drop(drop_cols, axis=1)

    return df_pairs


def create_edit_features(df_pairs):
    """Create all distance based features.
...
def create_edit_features_file(file_name):
    df = pd.read_csv(
    ...
    df["phoneNumbers"] = df["phoneNumbers"].str.replace("\[0","[",regex=True)
    df["phoneNumbers"] = df["phoneNumbers"].apply(eval)
    df["phoneNumbers"] = df["phoneNumbers"].apply(lambda x: list(set(x)))
    df["phoneNumbers"] = df["phoneNumbers"].map(str)
    df["phoneNumbers"] = df["phoneNumbers"].apply(clean_text)
    df["phoneNumbers"] = df["phoneNumbers"].replace("", np.nan)

    if config.is_inference == False:
        cols = [
            "clusterId", "supplier", "sourceNames", "category", "latitude",
            "brands", "phoneNumbers", "longitude", "houseNumber", "streets",
            "cities", "postalCode", "Id"
        ]
    else:
        cols = [
            "sourceNames", "supplier", "category", "latitude", "longitude",
            "brands", "phoneNumbers", "houseNumber", "streets", "cities",
            "postalCode", "Id"
        ]
    ...
    if config.is_inference == False:
        ...
        df_pairs = df_pairs.drop(
            ["Id1", "Id2", "clusterId1", "clusterId2", "placeId1", "placeId2"],
            axis=1)
    else:
        df_pairs = df_pairs.drop(["Id1", "Id2", "placeId1", "placeId2"],
                                 axis=1)

    # df_pairs = house_postal_match(df_pairs)
    df_pairs = add_lat_lon_distance_features(df_pairs)
    df_pairs = create_edit_features(df_pairs)
    return df_pairs
...
def create_edit_features_file_em(file_name):
    ...
    if config.is_inference == False:
        cols = [
            "clusterId", "sourceNames", "category", "latitude", "brands",
            "phoneNumbers", "longitude", "houseNumber", "streets", "cities",
            "postalCode", "supplier", "Id"
        ]
    else:
        cols = [
            "sourceNames", "category", "latitude", "longitude", "brands",
            "phoneNumbers", "houseNumber", "streets", "cities", "postalCode",
            "supplier", "Id"
        ]
    ...
    if config.is_inference == False:
        df_pairs["duplicate_flag"] = (df_pairs["clusterId1"]
                                      == df_pairs["clusterId2"]) * 1
        df_pairs = df_pairs.drop(["Id1", "Id2", "clusterId1", "clusterId2"],
                                 axis=1)
    else:
        df_pairs = df_pairs.drop(["Id1", "Id2"], axis=1)
...
def parallelize_create_edit_features_em(file_list):
    ...
    no_of_process = min(4, len(file_list))
...
def parallelize_create_edit_features(file_list):

    no_of_process = min(4, len(file_list))
    with ray.Pool(processes=no_of_process) as pool:
        results = list(
            tqdm(pool.imap(create_edit_features_file, file_list),
                 total=len(file_list)))
        new_df = pd.concat(results, axis=0, ignore_index=True)
    return new_df