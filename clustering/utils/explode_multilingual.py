...
from utils.text_clean import combine_category, convert_to_List_multilingual,latin_select
...
def explode_df_multilingual(df):
    ...
    df["category"] = df["category"].parallel_apply(
        lambda x: [i for i in x if i != "unspecified"]
    )

    df["brands"] = df["brands"].parallel_apply(lambda x: list(set(x)))
    ...
    df["brands"] = df["brands"].parallel_apply(lambda x: " ".join(set(x), key=x.index))
